...
if not st.session_state.game_active and not st.session_state.game_over:
    ...
    with col2:
        ...
        if st.button("🚀 Start Game", type="primary") and selected:
            ...
            st.session_state.trick_scores = pd.DataFrame(
                False,
                index=["Low Straight", "High Straight", "5 of a Kind"],
                columns=selected
            )
...
if st.session_state.game_active and not st.session_state.game_over:
    ...
    for i in range(10):
        with d_cols[i]:
            ...
            if c1.button("A", key=f"tA_{i}",
                         disabled=not st.session_state.first_roll_made,
                         type="primary" if inA else "secondary"):
            ...
            if c2.button("B", key=f"tB_{i}",
                         disabled=not st.session_state.first_roll_made,
                         type="primary" if inB else "secondary"):
...
# --- 7. SCORING ---
    ...
    tA_vals = sorted([st.session_state.dice[i] for i in st.session_state.trickA_indices], reverse=True)
    tB_vals = sorted([st.session_state.dice[i] for i in st.session_state.trickB_indices], reverse=True)
    ...
    def get_opts(dice, player):
        ...
        if (len(sorted_counts) == 2 and sorted_counts[0] >= 3 and sorted_counts[1] >= 2) \
           or (len(sorted_counts) == 1 and sorted_counts[0] == 5):
            opts.append("Full House")

        if check_dice == [1, 2, 3, 4, 5]:
            opts.append("Low Straight")

        if check_dice == [2, 3, 4, 5, 6]:
    ...
    with ca:
        st.markdown(
            f"<div class='bank-header'>Trick A ({len(tA_vals)}/5) &nbsp;&nbsp; {tA_vals if tA_vals else ''}</div>",
            unsafe_allow_html=True
        )
    ...
    with cb:
        st.markdown(
            f"<div class='bank-header'>Trick B ({len(tB_vals)}/5) &nbsp;&nbsp; {tB_vals if tB_vals else ''}</div>",
            unsafe_allow_html=True
        )
        sel_b = st.selectbox("Assign B:", get_opts(tB_vals, current_p), key="sB") if len(tB_vals) == 5 else None

    ready = len(tA_vals) == 5 and len(tB_vals) == 5
    label = "✅ Confirm Turn" if ready else "Assign all dice to confirm"

    if st.button(label, use_container_width=True,
                 disabled=not (sel_a and sel_b), type="primary"):
...
if st.session_state.game_active:
    ...
    st.data_editor(st.session_state.master_scores,
                   use_container_width=True, disabled=True)

    st.data_editor(st.session_state.trick_scores,
                   use_container_width=True, disabled=True)
